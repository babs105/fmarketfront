...
const EditBalayage = (props) => {
  ...
  const getbalayage = (id) => {
    console.log("ID", id);
    balayageService
      .get(id)
      .then((response) => {
        let dateBal = formatDateYYYYMMdd(new Date(response.data.date));
        setbalayage({ ...response.data, dateBal });

        console.log(response.data);
        Object.keys(initialbalayageState).forEach((field) => {
          setValue(field, response.data[field]);
        });
        setValue("date", dateBal);
      })
      .catch((e) => {
        console.log(e);
      });
  ...
  const onSubmit = (data) => {
    ...
    balayageService.update(id, data).then(
      (response) => {
        console.log("data new", response.data);
        setMessage("Editer balayage succes");
        setSuccessful(true);
        setLoading(false);
      },
      (error) => {
    ...
    <>
      <nav aria-label="breadcrumb">
        <ol className="breadcrumb">
          ...
          {/* <li className="breadcrumb-item">
            <Link to={`/trace/evenements/${balayage.eventid}`}>
              {balayage.localisation}
            </Link>
          </li> */}

          <li className="breadcrumb-item active ">Editer balayage</li>
        </ol>
      </nav>
      <div className="mt-2">
        <div className="d-flex justify-content-between">
          <h4 className="text-primary">Editer balayage</h4>
        </div>
        <hr className="mb-3" />
