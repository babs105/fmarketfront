...
function AddDetailAccident() {
  ...
  const getEvenement = (id) => {
    ...
    evenementService
      ...
      .then((response) => {
        ...
        setEvenement({ ...response.data, dateEvent });
        console.log(response.data);
        setValue("eventid", response.data.id);
        setValue("dateAcci", dateEvent);
        setValue("matriculeVhlImplique", response.data.matVehicule);
        setValue("typeVhlImplique", response.data.typeVehicule);
        setValue("causeAccident", response.data.causeEvent);
      })
      .catch((e) => {
        console.log(e);
      });
  ...
  const onSubmit = (data) => {
    ...
    detailAccidentService.create(data).then(
      (response) => {
    ...
    <div>
      <nav aria-label="breadcrumb">
        <ol className="breadcrumb">
          ...
          <li className="breadcrumb-item">
            <Link to={`/trace/evenements/${evenement.id}`}>
      ...
      <div className="d-flex justify-content-between">
        <h4 className="text-primary">Ajout Détail Accident</h4>
      </div>
      <hr className="mb-4" />
...
export default AddDetailAccident;
