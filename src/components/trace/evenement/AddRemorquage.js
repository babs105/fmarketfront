...
const AddRemorquage = (props) => {
  ...
  const validationSchema = Yup.object().shape({
    ...
    heureAppelROM: Yup.string().notRequired("H. appel ROM est obligatoire"),
  ...
  const getEvenement = (id) => {
    ...
    evenementService
      ...
      .then((response) => {
        ...
        setValue("dateRom", dateEvent);
        setValue("matVhlRemorque", response.data.matVehicule);
        setValue("catVhlRemorque", response.data.typeVehicule);
      })
      .catch((e) => {
        console.log(e);
      });
  ...
  const onSubmit = (data) => {
    ...
    remorquageService.create(data).then(
      (response) => {
